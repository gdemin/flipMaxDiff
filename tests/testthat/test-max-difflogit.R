...
test_that("Estimating logit parameters (without c++)", {
    # Aggregate
    result = FitMaxDiff(design = tech.design, version = rep(1, nrow(best)), best = best, worst = worst, names = names, fast = FALSE)
    ...
    result = FitMaxDiff(design = tech.design, version = rep(1, nrow(best)), best = best, worst = worst, subset = sub, names = names, fast = FALSE)
    ...
    result = FitMaxDiff(design = tech.design, version = rep(1, nrow(best)), best = best, worst = worst, subset = sub, weights = wgt, names = names, fast = FALSE)
